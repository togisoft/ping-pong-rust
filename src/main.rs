...
fn main() -> Result<(), String> {
    let win_config = WindowConfig::new().size(WIDTH, HEIGHT).title("Ping Pong").resizable(false);
...
fn draw(gfx: &mut Graphics, state: &mut State) {
    ...
    draw.rect(state.paddle_1.position(), (PADDLE_WIDTH, PADDLE_HEIGHT)).color(Color::PURPLE);
    draw.rect(state.paddle_2.position(), (PADDLE_WIDTH, PADDLE_HEIGHT)).color(Color::ORANGE);
    draw.image(&state.ball_img).position(state.ball.position().0, state.ball.position().1).size(BALL_SIZE, BALL_SIZE);


    draw.text(&state.font, "Press `R` to reset")
        .h_align_center()
        .v_align_middle()
        .position(width - 80.0, height - height + 15.0)
        .size(20.0);
    ...
    let score_one = format!("Player1:  {}", state.scores.0);
    draw.text(&state.font, &*score_one.clone())
        .h_align_left()
        .v_align_top()
        .position(width - width + 5.0, height + 5.0)
        .size(20.0);


    draw.text(&state.font, "TOGISOFT")
        .h_align_left()
        .v_align_top()
        .position(width - width * 0.60, height)
        .color(Color::BLUE)
        .size(40.0);

    let score_two = format!("Player2:  {}", state.scores.1);
    draw.text(&state.font, &*score_two.clone())
        .h_align_left()
        .v_align_top()
        .position(width - 105.0, height + 5.0)
        .size(20.0);
...
impl State {
    fn new(gfx: &mut Graphics) -> Self {
        ...
        let ball = gfx
            .create_texture()
            .from_image(include_bytes!("../assets/ball.png"))
            .build()
            .unwrap();
    ...
    fn reset(&mut self) {
        ...
        self.paddle_1.x = 10.0;
        self.paddle_1.y = HEIGHT as f32 * 0.5 - PADDLE_HEIGHT * 0.5;
        self.paddle_2.x = WIDTH as f32 - 10.0 - PADDLE_WIDTH;
        self.paddle_2.y = HEIGHT as f32 * 0.5 - PADDLE_HEIGHT * 0.5;
    }
}
